...
)
{
    ...
    while (1)
    {
        ...
        if (--i == 0)
        { // The emergency break. We will eventually terminate on
...
)
{
    ...
    while (1)
    {
        ...
        if (n & waitIRq)
        { // One of the interrupts that signal success has
          // been set.
            break;
        }
        if (n & 0x01)
        ...
        if (--i == 0)
        { // The emergency break. If all other condions fail we
    ...
    while (!uidComplete)
    {
        ...
        while (!selectDone)
        {
            ...
            if (currentLevelKnownBits >=
                32)
            { // All UID bits in this Cascade Level are known. This is
              // a SELECT.
